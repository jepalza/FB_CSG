...
sub my_exterior_noise cdecl(byval locals as double ptr)
    ' use three octaves 
    ...
    if (v>=-1.0) andalso (v<=0.4) then 'if the noise is in this range we move the point passed to us
        locals[0]+=v
        locals[1]+=v
        locals[2]+=v
	 endif
...
csg_exterior(@my_exterior_noise())
  csg_sphere(5)
  csg_sphere(5)
  csg_translate(5,0,0)
csg_end()

csg_save_stl("exterior.stl", .25, 5)